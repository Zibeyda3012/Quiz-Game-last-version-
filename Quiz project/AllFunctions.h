...
public:
	...
	{
		if (fullname.size() < 3) throw length_error("fullname must contain at least 3 symbols");
...
public:
	...
	void print()
	{
		cout << green << "*****Result*****" << white << endl << endl;
...
public:
	...
	string GetCorrectVariant()const { return _correct_answer; }
...
public:
	...
	void ShowAllPlayers()
	{
		...
		if (f.is_open())
		{
			...
			while (!f.eof())
			{
				...
				getline(f, incorrect, '~');
				getline(f, pass, '~');
				Player p(name, qName, stoi(correct), stoi(incorrect), stoi(pass));
				p.print();
				cout << endl << endl;
			}
		}
	...
	void printLeaderBoard(string quizName)
	{
		...
		cout << endl;
		cout << green << "Player Name" << "  " << "Correct" << "  " << "Incorrect" << "  " << "Pass" << white << endl;
	...
	void print(Question question, string name)
	{
		system("cls");
	...
	void checkAnswer(list<Question>questions, map<string, string>dict_answers, int& correct, int& incorrect, int& pass_count)
	{
		for (auto i : questions)
		{
			for (auto k : dict_answers)
			{
				if (i.GetQuestion() == k.first)
					if (k.second == "pass")
						pass_count++;
					else if (k.second == i.GetCorrectVariant())
						correct++;
					else
						incorrect++;
			}
	...
	void StartGame()
	{
		...
		map<string, string>dict_answers;
		for (auto i = newquestion.begin(); i != newquestion.end();)
		{
			print(*i, name);
			dict_answers[(*i).GetQuestion()] = my_answer;
			...
			if (step == 1) //next
			{
				if (i != (newquestion.end()))
				{
					...
					if (i == newquestion.end())
					{
						cout << endl << green << "you have reached the final question,if you want to finish please enter ~next~ or ~submit~,if you want to continue answering questions enter ~previous~" << white << endl;
						int a = GameChoiceOptions();


						if (a == 0 || a == 1)
						{
							checkAnswer(newquestion, dict_answers, correct, incorrect, pass_count);
